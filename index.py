...
class JanelaPrincipal:
    ...
    def button_atualizar_acao(self):
        ...
        for item in resultado:
            ...
            media_formatada = self.formatar_media(media) if media is not None else 'N/A'
            self.tv.insert('', tk.END, values=(id_nota, nome, materia, av1, av2, av3, media_formatada))
    ...
    def button_pesquisar_acao(self):
        ...
        for item in resultado:
            id_nota, nome, materia, av1, av2, av3, media = item
            self.tv.insert('', tk.END, values=(id_nota, nome, materia, av1, av2, av3, media))
    ...
    def id_selecionado(self):
        try:
            ...
            messagebox.showinfo(title='ERRO', message='Selecione um elemento a ser editado')
            return None
    ...
    def iniciar_interface(self):
        ...
        self.tv = ttk.Treeview(self.janela, columns=('id', 'nome', 'materia', 'av1', 'av2', 'av3', 'media'), show='headings')
        ...
        # Botões
        button_atualizar = tk.Button(self.janela, text='ATUALIZAR', command=self.button_atualizar_acao, bg='lightgrey', fg='black', width=15, height=2)
        button_atualizar.place(x=10, y=10, width=80, height=30)

        button_criar = tk.Button(self.janela, text='CRIAR', command=self.button_criar_acao, bg='lightgrey', fg='black', width=15, height=2)
        button_criar.place(x=10, y=45, width=80, height=30)

        button_pesquisar = tk.Button(self.janela, text='PESQUISAR', command=self.button_pesquisar_acao, bg='lightgrey', fg='black', width=15, height=2)
        button_pesquisar.place(x=10, y=80, width=80, height=30)

        self.texto_pesquisa = tk.Text(self.janela, bg='lightgrey', fg='black', width=20, height=2)
        self.texto_pesquisa.place(x=10, y=110, width=80, height=20)

        # Botão editar
        button_editar = tk.Button(janela, text='EDITAR', command=self.button_editar_acao, bg='lightgrey', fg='black', width=15, height=2)
        button_editar.place(x=10, y=135, width=80, height=30)

        button_remover = tk.Button(self.janela, text='REMOVER', command=self.button_remover_acao, bg='lightgrey', fg='black', width=15, height=2)
